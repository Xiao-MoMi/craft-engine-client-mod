...
public interface ServerCustomPacket extends FabricPacket {

    static <B extends ByteBuf, T extends ServerCustomPacket> NetworkCodec<B, T> codec(NetworkMemberEncoder<B, T> networkMemberEncoder, NetworkDecoder<B, T> networkDecoder) {
        return NetworkCodec.ofMember(networkMemberEncoder, networkDecoder);
    }

    default void write(FriendlyByteBuf friendlyByteBuf) {
        @SuppressWarnings("unchecked")
        var codec = (NetworkCodec<FriendlyByteBuf, ServerCustomPacket>) codec();
        codec.encode(friendlyByteBuf, this);
    }

    @NotNull ResourceLocation id();

    NetworkCodec<FriendlyByteBuf, ? extends ServerCustomPacket> codec();

    PacketType<? extends ServerCustomPacket> getType();
